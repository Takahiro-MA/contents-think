...
- **A系統（物語形式・CLAUDE.md＋docs/21準拠）**: 田中さんシリーズ・藤井さんシリーズの全書籍
  （01〜03, 05, 07〜10 の既刊、および進行中・仕様済みの物語形式書籍すべて）
- **B系統（AI共著・project_rules駆動。テンプレ使用禁止ルールはこの系統内のみ）**:
  `../kindle2/04_tdd-book/`（※原稿はA系統形式で出版済み。project_rulesは内容検討段階で使用）、
  `../kindle2/X3_LLM_in_office/`
- **未分類**: X1, X2, X4〜X7（休止中の旧企画。再開時に判定する）
...
| 10 | SQL | `sql/` | 9 | Phase 1A待ち（**物語建て直し先行**） | 1→1R→**1A待ち** | **2026-07-09著者判断: 需要3位×物語10位のミスマッチ。1Aの前に物語エンジンの建て直しをやる**。**2026-08-21著者判断で方向確定: 構文カタログを主軸に置かず「集合思考の転換」を背骨、「問いの精密化（問いがクエリを呼ぶ）」を各章エンジンにする。詳細は `sql/notes/2026-08-21-rebuild-direction.md`**。主な書き換え対象はhub §6・§11 |
...
|---|--------|-------------|------|-------|---------|-------|------|
| 1 | 断酒 | `sobriety/` | 8 | **企画再検討（Phase 0相当に差し戻し）** | 1→1R→2→3→4（実績として保持） | `../kindle2/06_sobriety-book/epub/` | **2026-08-21著者判断: 根本から考え直すため進捗を差し戻し**。既存初稿・Claudeレビュー（43件中12件修正済み）は資産として保持。再開時は企画の練り直し（テンプレ脱却）から |
